...
describe("Recintos do Zoologico", () => {
  ...
  test("Deve rejeitar quantidade inválida (zero)", () => {
    const resultado = new RecintosZoo().analisaRecintos("MACACO", 0);
    expect(resultado.erro).toBe("Quantidade inválida");
    expect(resultado.recintosViaveis).toBeFalsy();
  });

  test("Deve rejeitar quantidade inválida (número dentro de string)", () => {
    const resultado = new RecintosZoo().analisaRecintos("MACACO", "2");
    expect(resultado.erro).toBe("Quantidade inválida");
    expect(resultado.recintosViaveis).toBeFalsy();
  });

  test("Deve rejeitar quantidade inválida (não é um número)", () => {
    const resultado = new RecintosZoo().analisaRecintos("MACACO", "teste");
    expect(resultado.erro).toBe("Quantidade inválida");
    expect(resultado.recintosViaveis).toBeFalsy();
  });
